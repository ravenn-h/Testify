...
async function process(sock, messageInfo) {
  ...
  try {
    ...
    if (!dataGroupSettings) {
      logWithTime(
        "System",
        `Data grup not found atau fitur belum diaktifkan.`
      );
      return true;
    }
    ...
    // Fungsi untuk processing kick anggota
    ...
    const sendText = async (text, isquoted = false) => {
      logTracking(`Handler - sending pesan teks (${command})`);
      if (isquoted) {
      ...
      if (hasBadwordGroup.status || hasBadwordGlobal.status) {
        ...
        if (result.status === "blocked") {
          ...
          try {
            ...
            console.error(
              "❗ _An error occurred, badword action failed. Make sure the Bot is an admin_"
            );
            await sendText(
              "❗ _An error occurred, tindakan badword gagal. Make sure Bot Adalah admin_"
            );
          }
        }
        return false;
    ...
    if (
      ...
      if (result.status === "blocked") {
        ...
        try {
          ...
          console.error(
            "An error occurred while processing tindakan spam:",
            error
          );
          await sendText("❗ _An error occurred, spam action failed._");
        }
    ...
    if (fitur?.onlyadmin) {
      if (command === "debug") {
        ...
        return false; // Stop setelah displaying status debug
      }
    ...
    if (fitur?.antitagmeta && isGroup && isTagMeta) {
      let warningMessage = "⚠️ @sender _Meta AI tag detected in this group_";
      if (warningMessage) {
    ...
    if (!isAdmin && fitur?.antiforward && isGroup && isForwarded) {
      let warningMessage = "⚠️ @sender _Forwarded message detected_";
      if (warningMessage) {
        warningMessage = warningMessage.replace(
          "@sender",
          `@${sender.split("@")[0]}`
    ...
    console.error("An error occurred pada proses Handler.js:", error.message);
  }
